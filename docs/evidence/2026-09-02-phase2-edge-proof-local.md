...
Node v22.22.3. **159 tests, 159 pass, 0 fail** (measured 2026-09-02, after fix
round 7). The tally by round: 47/47 pre-review, 77/77 after round 1, 82/82
mid-round-2 (the partial pass), 100/100 after round 2, 116/116 after round 3,
124/124 after round 4, 140/140 after round 5, 158/158 after round 6, 159/159
after round 7 (current, both runner modes). `edge/package.json` declares
...
159/159 in both modes as of round 7 (it was 0 pass / 5 fail before round 4;
140/140 at round 5; 158/158 at round 6).
...
| Warning thresholds apply to every meter kind | The warn branch reaches ceilings too, diagnostic only: no shed action, `breached: false`. At the time this row was written the state was published as a `warning: true`/`false` boolean on the reading; **round 7 removed that key from the wire** because the frozen `MeterReading` contract has no such field, and a consumer derives the identical condition from `value`, `warning_threshold` and `breached: false` (see the round-7 table below) | `a ceiling between its warning and its hard stop is derivable as a warning with no shed action`, `a warning on a budget still emits its shed action, and a breach is not also a warning` |
| Envelope shape is pinned byte for byte | `edge/fixtures/limit-receipt.sample.json` holds the full serialization from fixed inputs, compared as a string; the test FAILS with `ENOENT` when the committed sample is missing rather than writing one. It pins a reading between its warning and hard-stop thresholds, an unread meter as `null`, and the four ownership keys | `the receipt envelope matches the committed sample byte for byte`, `the committed sample carries the four ownership keys the envelope contract requires` |
...
| Readings conform to the frozen `MeterReading` field set (round 7) | The diagnostic `warning` boolean the edge published on every reading was never in `curator/contracts/receipt.py`. The Python-side contract test (`tests/test_edge_receipt_sample.py` in the main checkout, added at merge time) failed with `LimitReceipt.readings[0]: unknown field warning`. The lead's ruling: the contract stays frozen, the edge stops emitting the flag. `meter.js` no longer sets it; a consumer derives the identical between-thresholds condition from `value`, `warning_threshold` and `breached: false`. The warn branch's other behavior (no shed action for ceiling meters, the shed-action/reason-code effects for budgets) is unchanged | `every emitted reading has exactly the nine frozen MeterReading keys and no others`; `a well-formed reading carries all nine required fields, every one the right type`; the merge-time Python pin, `test_edge_sample_receipt_satisfies_the_frozen_limit_receipt_contract` in `tests/test_edge_receipt_sample.py`, run against this worktree's regenerated `edge/fixtures/limit-receipt.sample.json` and PASSED | B |
...
here: this worktree carries no Python. One key the sample emits is ahead of the
frozen contract by design (`ReceiptEnvelope.actor_kind` and `user_id` from the
adjudicated Ownership change); `MeterReading` carries no such gap as of round 7,
which removed the diagnostic `warning` flag from the wire rather than widening
the contract, so every reading key already matches
`curator/contracts/receipt.py`. The same note is written next to the fixture path in `edge/meter.test.js` so it
...
| A reading silently dropped `meter_kind` and `unit` when the spec omitted them, because `JSON.stringify` drops an `undefined` value | `meter.js` validates all `MeterReading` fields present with the right type before serialization (nine as of round 7; ten at the time this row was written, before the diagnostic `warning` flag was removed from the wire); a defect is a typed `ReceiptInputError`, which the route turns into a 503. A spec that is not an object at all is the same refusal instead of a `TypeError` | `a meter spec missing its unit is a refusal, never a reading with the key dropped`; `a meter spec that is not an object at all is a refusal, not a TypeError`; `a well-formed reading carries all nine required fields, every one the right type` | B |
...
that matters. Reading shape is enforced for the same reason: all nine
`MeterReading` fields must be present with the right type before serialization,
and no others, because `JSON.stringify` drops an `undefined` value and a spec
missing its `unit` would otherwise publish a row with no `unit` key at all.
