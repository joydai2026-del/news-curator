...
- **Retained-corpus ingest, deploy order (M2.1 Phase 1).** `supabase/migrations/202609170001_m2_retained_overlay.sql` adds `m2_apply_retained_overlay` and **must be applied, with the PostgREST schema cache reloaded, BEFORE the ingest code that calls it reaches `main`.** In the window between the two, the hourly run writes the corpus, pairs, persists its decisions and then finds no such function: PostgREST answers 404. The run degrades to `::warning::overlay not applied: HTTP 404, m2_apply_retained_overlay is not deployed yet ...` and exits 0, so the corpus keeps updating and only the overlay waits, but the warning stays in every run's log until the migration lands. Apply it, then merge.
