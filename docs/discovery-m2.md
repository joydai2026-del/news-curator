...
These are local historical source replays. Missing history is unknown, not empty. The CLI cannot settle an owner edition and its verification proves internal consistency, not source authenticity. Output is created with mode600 and never overwrites an existing file. The committed fixture is captured public news; controlled negative test mutations must not become a news demonstration.

## Private serving path

The existing protected personalization job runs `scripts/materialize_discovery.py` with its configured owner and Supabase service credentials. It obtains a source-only prior capture from a bounded search of successful main-workflow artifacts, fetches the actual owner profile and complete retained edition history, rebuilds the receipt, verifies all input bindings and finalizes only PASS. It exports no private artifact to Actions or Pages. Unknown or failed inputs preserve the previous private edition.

Retries use a stable identity derived from owner, current/prior captures, profile revision and full profile fingerprint, policy, language, ranking/dedup configuration, git revision and actual engine fingerprint. A service-only identity lookup reconciles an uncertain commit before any retry. SQL checks the same identity and atomically stores the edition with its source facts.
...
- Deployment migration: `supabase/migrations/202609090001_discovery_lanes.sql`, after the existing M1 migrations.
...
Release is separate from build completion. Require reviewed code, PostgreSQL17.11 CI, a genuine owner-bound passing edition and live owner read/save verification before acceptance. The captured source-only edition in the build record fails its bands and is not a release candidate.
