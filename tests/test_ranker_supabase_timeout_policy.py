...
@pytest.mark.parametrize(("retries", "worst_case"), [(0, 170), (1, 180), (2, 190)])
...
def test_boot_keeps_the_claim_budget_to_one_owner_state_read(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="reached tokenizer"):
        runtime.build_application(environ={**_boot_env(), FUNCTION_TIMEOUT_ENV: "181"})
    assert claim_counts == [23]
