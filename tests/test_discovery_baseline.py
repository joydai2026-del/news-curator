...
def capture_root(tmp_path):
    """Materialize the config that the immutable capture was made with."""
    config_root = tmp_path / 'capture-config'
    config_root.mkdir()
    for name in ('topics.yaml', 'sources.yaml'):
        shutil.copyfile(CAPTURE_CONFIG / name, config_root / name)
    policy_dir = config_root / 'config'
    policy_dir.mkdir()
    (policy_dir / 'discovery-policy-r2.yaml').write_bytes(
        (ROOT / 'config/discovery-policy-r2.yaml').read_bytes()
    )
    return config_root
...
def test_prior_successful_capture_validated_and_created_private(captures, capture_root, tmp_path):
    call, calls = transport(captures)
    output = tmp_path/'output.json'
    assert baseline.fetch_baseline(capture_root, CAPTURE, output, 'owner/repo', '10', run=call)
    assert output.read_bytes() == captures.read_bytes()
    assert output.stat().st_mode & 0o777 == 0o600
    assert '--name' in calls[-1] and 'source-snapshot' in calls[-1]
    with pytest.raises(ValueError):
        baseline.fetch_baseline(capture_root, CAPTURE, output, 'owner/repo', '10', run=call)


@pytest.mark.parametrize('kwargs', [
    {'conclusion': 'failure'}, {'branch': 'feature'}, {'repository': 'foreign/repo'}, {'run_id': 10},
])
def test_other_runs_never_used(captures, capture_root, tmp_path, kwargs):
    call, calls = transport(captures, **kwargs)
    output = tmp_path/'output.json'
    assert not baseline.fetch_baseline(capture_root, CAPTURE, output, 'owner/repo', '10', run=call)
    assert len(calls) == 1 and not output.exists()


def test_equal_clock_not_a_previous_capture(captures, capture_root, tmp_path):
...
def test_transport_failure_stays_unavailable(captures, capture_root, tmp_path):
    def unavailable(args, **kwargs):
        raise subprocess.CalledProcessError(1, args, stderr='Controlled secret-shaped error must stay private')
    assert not baseline.fetch_baseline(capture_root, CAPTURE, tmp_path/'output.json', 'owner/repo', '10', run=unavailable)


def test_first_edition_selects_oldest_valid_capture_independent_of_api_order(capture_root, tmp_path, monkeypatch):
    ...
    assert baseline.fetch_baseline(capture_root, CAPTURE, output, 'owner/repo', '10', attempts=2, run=call)
    assert output.read_bytes() == oldest


def test_existing_edition_selects_newest_capture_at_or_before_anchor(capture_root, tmp_path, monkeypatch):
    current, now = current_capture()
    freeze_loader(monkeypatch, now)
    ...
    assert baseline.fetch_baseline(
        capture_root, CAPTURE, output, 'owner/repo', '10', attempts=3,
        anchor_before=now - timedelta(hours=2), run=call,
    )
    assert output.read_bytes() == anchor_match


def test_invalid_clock_and_configuration_captures_are_excluded(capture_root, tmp_path, monkeypatch):
    current, now = current_capture()
    freeze_loader(monkeypatch, now)
    ...
    assert baseline.fetch_baseline(capture_root, CAPTURE, output, 'owner/repo', '10', attempts=3, run=call)
    assert output.read_bytes() == valid


def test_supplied_updates_window_rejects_older_capture(capture_root, tmp_path, monkeypatch):
    current, now = current_capture()
    freeze_loader(monkeypatch, now)
    older = snapshot_bytes(tmp_path, current, now - timedelta(hours=2))
    call, _ = multi_transport({'order': [9], 'bytes': {9: older}})
    policy = deepcopy(baseline.load_discovery_policy(capture_root / 'config/discovery-policy-r2.yaml'))
    policy['windows']['updates'] = 1

    assert not baseline.fetch_baseline(
        capture_root, CAPTURE, tmp_path / 'baseline.json', 'owner/repo', '10',
        attempts=1, policy=policy, run=call,
    )


def test_complete_window_includes_candidate_beyond_old_hourly_cutoff(capture_root, tmp_path, monkeypatch):
    current, now = current_capture()
    freeze_loader(monkeypatch, now)
    ...
    assert baseline.fetch_baseline(capture_root, CAPTURE, output, 'owner/repo', '27', run=call)
    assert output.read_bytes() == captures['bytes'][1]


def test_saturated_workflow_page_fails_before_download(capture_root, tmp_path, monkeypatch):
    current, now = current_capture()
    freeze_loader(monkeypatch, now)
    ...
    assert not baseline.fetch_baseline(capture_root, CAPTURE, tmp_path / 'baseline.json', 'owner/repo', '101', run=call)
    assert len(calls) == 1


def test_query_is_limited_to_the_declared_created_window(capture_root, tmp_path, monkeypatch):
    current, now = current_capture()
    freeze_loader(monkeypatch, now)
    raw = snapshot_bytes(tmp_path, current, now - timedelta(minutes=1))
    call, calls = multi_transport({'order': [9], 'bytes': {9: raw}})

    assert baseline.fetch_baseline(capture_root, CAPTURE, tmp_path / 'baseline.json', 'owner/repo', '10', run=call)
    query = calls[0][2]
    assert 'per_page=100' in query and 'created=' in query
    assert '%3A' in query and '..' in query


def test_baseline_age_uses_the_materializer_evaluation_clock(capture_root, tmp_path, monkeypatch):
    current, now = current_capture()
...
def test_partial_workflow_response_fails_before_download(capture_root, tmp_path, monkeypatch):
    current, now = current_capture()
    freeze_loader(monkeypatch, now)
    calls = []
    ...
    assert not baseline.fetch_baseline(capture_root, CAPTURE, tmp_path / 'baseline.json', 'owner/repo', '10', run=call)
    assert len(calls) == 1


def test_malformed_row_refuses_complete_page_before_valid_candidate_download(capture_root, tmp_path, monkeypatch):
    current, now = current_capture()
    freeze_loader(monkeypatch, now)
    calls = []
