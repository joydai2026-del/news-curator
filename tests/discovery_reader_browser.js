...
await page.locator('button[data-discovery-lane="hot"]').click();
assert.equal(await page.locator('#discovery-status').textContent(),'Fewer qualified Hot stories were selected for this edition.');
await page.locator('button[data-discovery-lane="surprise"]').click();
assert.equal(await page.locator('.card:visible').count(),1);
assert.match(await page.locator('#discovery-status').textContent(),/Fewer qualified Surprise stories were selected for this edition\.$/);
