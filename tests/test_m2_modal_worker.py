...
def test_worker_has_bounded_config_and_same_secret(monkeypatch, tmp_path):
    values = _required(tmp_path, preparation_enabled=True) | {
...
def test_worker_rejects_invalid_operational_config(monkeypatch, tmp_path, key, value):
    with pytest.raises(ValueError):
        _load(monkeypatch, **(_required(tmp_path, preparation_enabled=True) | {
            "NEWS_CURATOR_MODAL_PREPARATION_WORKER_ENABLED": "true",
            key: value,
        }))
...
def test_scrub_handler_reports_only_aggregate_and_runs_when_paid_worker_disabled(monkeypatch, capsys):
    ...
    assert set(logged) == {"event", "scrubbed", "duration_ms"}
    assert logged["scrubbed"] == 2
...
def test_handler_processes_only_configured_batch_and_logs_no_ids(monkeypatch, capsys):
    ...
    logged = json.loads(capsys.readouterr().out)
    assert set(logged) == {"event", "jobs", "outcomes", "duration_ms"}
    assert logged["outcomes"] == {"ready": 2}
...
def test_scrub_handler_rejects_private_payload_in_count_without_logging(monkeypatch, capsys):
    from curator.recommendation import modal_handlers, runtime
...
def test_worker_handler_rejects_private_payload_in_outcome_without_logging(monkeypatch, capsys):
    from curator.recommendation import modal_handlers, preparation_worker, runtime
    private_id = "private-owner-story-id"
    service = types.SimpleNamespace(_store=object(), _adapter=object(), _policy=object())
    monkeypatch.setattr(runtime, "build_application", lambda: types.SimpleNamespace(_service=service))
    monkeypatch.setattr(preparation_worker, "process_one_preparation",
                        lambda **kwargs: f"failed:{private_id}")
    monkeypatch.setenv("NEWS_CURATOR_MODAL_PREPARATION_BATCH_SIZE", "1")
    with pytest.raises(ValueError, match="invalid outcome"):
        modal_handlers.prepare_next_run()
    assert private_id not in capsys.readouterr().out


@pytest.mark.parametrize("policy_enabled,worker_enabled", [
    (True, "false"),
    (False, "true"),
])
def test_staged_preparation_policy_must_match_worker_registration(
        monkeypatch, tmp_path, policy_enabled, worker_enabled):
    values = _required(tmp_path, preparation_enabled=policy_enabled) | {
        "NEWS_CURATOR_MODAL_PREPARATION_WORKER_ENABLED": worker_enabled,
    }
