...
begin
  ...
  return query
  with keyed as (
    select o.story_id, o.language, o.published_at, o.source_id,
      public.m2_story_dedupe_key(o.title) as dedupe_key
    ...
    from chosen c
    -- Preserve the existing order: dedupe first, owner filters and cursor next.
    where (p_before_published_at is null or c.published_at < p_before_published_at
           or (c.published_at = p_before_published_at and c.story_id < p_before_story_id))
      and c.story_id <> all(coalesce(p_excluded_story_ids, '{}'::text[]))
      and (not p_hide_already_opened or not exists (
        select 1 from public.user_story_state opened
        where opened.user_id = p_owner_id and opened.story_id = c.story_id
          and opened.read_at is not null))
      and c.source_id <> all(coalesce(p_suppressed_sources, '{}'::text[]))
      and not exists (
        select 1 from public.retained_corpus_categories rc
        where rc.story_id = c.story_id
          and rc.category_id = any(coalesce(p_suppressed_topics, '{}'::text[])))
    order by c.published_at desc, c.story_id desc
    limit p_limit
  )
