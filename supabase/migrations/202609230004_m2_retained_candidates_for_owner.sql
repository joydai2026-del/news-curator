...
-- Owner-opened filtering follows dedupe and precedes LIMIT, so an opened head
-- cannot consume the fetch budget. The deployed filtered RPC stays unchanged
-- for rollback. Deploy this migration before the new application route.
create or replace function public.m2_retained_candidates_for_owner(
...
revoke all on function public.m2_retained_candidates_for_owner(uuid, boolean, text, text, text, text[], text[], integer, integer, integer, integer, timestamptz, text, integer, integer, integer, text[], text[], text[])
  from public, anon, authenticated;
grant execute on function public.m2_retained_candidates_for_owner(uuid, boolean, text, text, text, text[], text[], integer, integer, integer, integer, timestamptz, text, integer, integer, integer, text[], text[], text[])
  to service_role;
