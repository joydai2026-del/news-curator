...
# The image carries no sources.yaml (that file configures collection, not the
# ranker), so the ONE value the boot-time retention cross-check needs is
# generated into the image context under this name by
# scripts/prepare_ranker_image_context.py. Same shape as sources.yaml, so one
# reader serves a local checkout and a container alike.
RETENTION_INPUTS_FILE = "config/retention-inputs.yaml"
...
def boot_retention_days(*candidates: str | Path) -> int:
    """The retention window for check 8, from the first candidate file present.

    Refuses instead of returning None. `configured_retention_days` answers None
    for an absent file so a deployment that has not adopted the key still boots,
    and in the container that turned check 8 into a validator that never ran:
    sources.yaml is not in the image, so the corpus-versus-window check was
    silently skipped on every boot. A boot contract that can be skipped by a
    missing file is not a contract.
    """
    for candidate in candidates:
        path = Path(candidate)
        if not path.is_file():
            continue
        value = configured_retention_days(path)
        if value is None:
            raise CompositionPolicyError(
                f"{path} has no coverage.observations_retention_days, so the retention "
                "cross-check cannot run. Set the key rather than removing the check.")
        return value
    raise CompositionPolicyError(
        "retention inputs are missing: none of " + ", ".join(str(c) for c in candidates) +
        " exists, so the corpus-versus-window cross-check cannot run. In an image this "
        f"file is generated by scripts/prepare_ranker_image_context.py as {RETENTION_INPUTS_FILE}.")
